...
type Tab = "devices" | "friends" | "transfers" | "settings";
...
function App() {
  ...
  const [chatMessages, setChatMessages] = useState<import("./types").ChatEntry[]>([]);
  ...
  const refreshTrustedDeviceList = () => {
    api.getTrustedDevices().then(setTrustedDevices).catch(() => {});
    api.getTrustedDeviceList().then(setTrustedDeviceList).catch(() => {});
  };
  ...
  useEffect(() => {
    ...
    const setupListeners = async () => {
      ...
      const unlisten3 = await listen<{ id: string; bytes_transferred: number; speed: number }>("transfer-progress", (event) => {
        updateTransferProgress(event.payload.id, event.payload.bytes_transferred, event.payload.speed);
      });
      unlisteners.push(unlisten3);

      type TransferPayload = { id: string; file_name?: string; error?: string };

      const unlisten4 = await listen<TransferPayload>("transfer-complete", (event) => {
      ...
      const unlisten5 = await listen<TransferPayload>("transfer-error", (event) => {
        updateTransferError(event.payload.id, event.payload.error || "");
        refreshTransfers();
        setStatusMessage("传输出错");
        const fileName = event.payload.file_name || "";
        notify("传输出错", fileName ? `${fileName}: ${event.payload.error}` : event.payload.error);
      });
      unlisteners.push(unlisten5);

      // Listen for real-time chat messages (with dedup)
      const unlisten6 = await listen<import("./types").ChatEntry>("chat-message", (event) => {
      ...
      const unlisten7 = await listen<{ device_id: string; online: boolean }>("device-status", (event) => {
        setOnlineDeviceIds(prev => {
          if (event.payload.online) {
            return prev.includes(event.payload.device_id) ? prev : [...prev, event.payload.device_id];
          } else {
      ...
      <div className="app-body">
        ...
        <div className="content-panel">
          ...
          <main className="app-content">
            ...
            {activeTab === "settings" && (
              <Settings onStatusMessage={setStatusMessage} theme={theme} onToggleTheme={toggleTheme} />
            )}
